...
PRED_ROOT=$PROJ_ROOT/predictions/preco-$timestamp
mkdir -p $PRED_ROOT
#mkdir -p $SAVE_ROOT

for cname in train_spanbert_base_preco_mult \
             train_spanbert_large_preco_mult \
             train_bert_base_preco_mult \
             train_bert_large_preco_mult \
             train_spanbert_base_preco_mult_gold \
             train_spanbert_large_preco_mult_gold \
             train_bert_base_preco_mult_gold \
             train_bert_large_preco_mult_gold ;
do
  MEM="32g"
  SAVE="${SAVE_ROOT}/${cname}"
  mkdir -p ${SAVE}
  SCRIPT="${JOBSCRIPTS}/eval-${exp}.${cname}.sh"
  SLURM="${JOBSCRIPTS}/eval-${exp}.${cname}.slrm"
  output_prefix="$SAVE/eval-${exp}-$timestamp"
  echo "#!/bin/sh" > ${SCRIPT}
  echo "#!/bin/sh" > ${SLURM}
  ...
  experiments_file="experiments.conf"
